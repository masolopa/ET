...
export class MirutaPage implements OnInit {
  ...
  async loadMap() {
    await this.geo
      .getCurrentPosition()
      .then((position: { lat: number; lng: number } | null) => {
        if (position) {
  ...
  goToDUOC() {
    this.translate.get('MiRuta.Label.InstitutoDUOC').subscribe((translatedTitle: string) => {
      this.goToPosition(
        -33.44703,
        -70.65762,
        50, translatedTitle
      ...
      .then((position: { lat: number; lng: number } | null) => {
        if (position) {
          this.translate
            .get('MiRuta.Label.MiUbicacion')
            .subscribe((translatedTitle: string) => {
              this.goToPosition(position.lat, position.lng, 50, translatedTitle);
              this.getRoute(
                { lat: position.lat, lng: position.lng },
                { lat: -33.44703, lng: -70.65762 },
                'walking'
              );
            });
  ...
  getRoute(
    ...
    this.http.get(url).subscribe((response: any) => {
      if (this.map) {
        ...
        const routeLatLngs = routeCoords.map((coord: [number, number]) => [
          coord[1],
          coord[0],
        ]);
